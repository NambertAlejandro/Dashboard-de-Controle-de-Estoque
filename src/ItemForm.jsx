...
export default function ItemForm({ item, enabledTypes, categories, onCancel, onSave, onImportMany }) {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    {type?.stock && <section><h3 className="text-sm font-semibold text-slate-700 mb-3">Recebimento e saldo</h3>
      ...
      <Field id="item-received" label={item ? <>Quantidade recebida neste lote <span className="ml-1 rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600">Não editável</span></> : 'Quantidade recebida'}><input id="item-received" type="number" min="0" step="any" required value={form.quantityReceived} onChange={set('quantityReceived')} readOnly={!!item} className={`${inputClass} ${item ? 'cursor-not-allowed bg-slate-100 text-slate-500' : ''}`} /></Field>
      <Field id="item-lost" label={item ? <>Quantidade perdida <span className="ml-1 rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600">Não editável</span></> : 'Quantidade perdida'} hint="Quebras, avarias ou itens inutilizados no recebimento."><input id="item-lost" type="number" min="0" step="any" required value={form.lostQuantity} onChange={set('lostQuantity')} readOnly={!!item} className={`${inputClass} ${item ? 'cursor-not-allowed bg-slate-100 text-slate-500' : ''}`} /></Field>
